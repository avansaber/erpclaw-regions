...
def seed_ca_defaults(conn, args):
    ...
    audit(conn, "erpclaw-region-ca", "ca-seed-ca-defaults", "company", company_id,
...
def setup_gst_hst(conn, args):
    ...
    audit(conn, "erpclaw-region-ca", "ca-setup-gst-hst", "company", company_id,
...
def seed_ca_payroll(conn, args):
    ...
    audit(conn, "erpclaw-region-ca", "ca-seed-ca-payroll", "company", company_id,
...
def available_reports(conn, args):
    ...
    reports = [
        {"name": "GST/HST Return (GST34)", "action": "ca-generate-gst-hst-return", "description": "Monthly/quarterly GST/HST return"},
        {"name": "QST Return", "action": "ca-generate-qst-return", "description": "Quebec Sales Tax return (QC only)"},
        {"name": "T4", "action": "ca-generate-t4", "description": "Statement of Remuneration Paid (annual per employee)"},
        {"name": "T4A", "action": "ca-generate-t4a", "description": "Statement of non-employment income"},
        {"name": "ROE", "action": "ca-generate-roe", "description": "Record of Employment (on separation)"},
        {"name": "PD7A", "action": "ca-generate-pd7a", "description": "Monthly remittance statement (CPP + EI + tax)"},
        ...
        {"name": "Tax Rates", "action": "ca-list-tax-rates", "description": "All provinces GST/HST/PST/QST rates"},
        {"name": "Input Tax Credits", "action": "ca-compute-itc", "description": "ITC from purchase invoices"},
        {"name": "Sales Tax Calculator", "action": "ca-compute-sales-tax", "description": "All-in-one province tax calculator"},
        {"name": "Payroll Deductions", "action": "ca-compute-total-payroll-deductions", "description": "CPP/EI/tax all-in-one"},
    ]
...
ACTIONS = {
    # Validation (2)
    "ca-validate-business-number": validate_business_number,
    "ca-validate-sin": validate_sin,
    # Tax computation (7)
    "ca-compute-gst": compute_gst,
    "ca-compute-hst": compute_hst,
    "ca-compute-pst": compute_pst,
    "ca-compute-qst": compute_qst,
    "ca-compute-sales-tax": compute_sales_tax,
    "ca-list-tax-rates": list_tax_rates,
    "ca-compute-itc": compute_itc,
    # Seed / Setup (4)
    "ca-seed-ca-defaults": seed_ca_defaults,
    "ca-setup-gst-hst": setup_gst_hst,
    "ca-seed-ca-coa": seed_ca_coa,
    "ca-seed-ca-payroll": seed_ca_payroll,
    # Payroll (8)
    "ca-compute-cpp": compute_cpp,
    "ca-compute-cpp2": compute_cpp2,
    "ca-compute-qpp": compute_qpp,
    "ca-compute-ei": compute_ei,
    "ca-compute-federal-tax": compute_federal_tax,
    "ca-compute-provincial-tax": compute_provincial_tax,
    "ca-compute-total-payroll-deductions": compute_total_payroll_deductions,
    "ca-payroll-summary": ca_payroll_summary,
    # Compliance (6)
    "ca-generate-gst-hst-return": generate_gst_hst_return,
    "ca-generate-qst-return": generate_qst_return,
    "ca-generate-t4": generate_t4,
    "ca-generate-t4a": generate_t4a,
    "ca-generate-roe": generate_roe,
    "ca-generate-pd7a": generate_pd7a,
    # Reports (3)
    "ca-tax-summary": ca_tax_summary,
    "ca-available-reports": available_reports,
    "status": status_action,
}


def main():
    parser = SafeArgumentParser(description="ERPClaw Canada Regional Skill")
