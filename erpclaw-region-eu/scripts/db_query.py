...
def seed_eu_defaults(conn, args):
    ...
    conn.commit()
    audit(conn, "erpclaw-region-eu", "eu-seed-eu-defaults", "company", cid,
...
def setup_eu_vat(conn, args):
    ...
    conn.commit()
    audit(conn, "erpclaw-region-eu", "eu-setup-eu-vat", "company", cid,
...
def seed_eu_coa(conn, args):
    ...
    conn.commit()
    audit(conn, "erpclaw-region-eu", "eu-seed-eu-coa", "company", cid, new_values={"accounts": created})
    conn.commit()
    ok({
...
def available_reports(conn, args):
    ...
    ok({
        "reports": [
            {"name": "VAT Return", "action": "eu-generate-vat-return", "description": "Member state VAT return"},
            {"name": "EC Sales List", "action": "eu-generate-ec-sales-list", "description": "Intra-community B2B supply listing"},
            {"name": "SAF-T Export", "action": "eu-generate-saft-export", "description": "OECD Standard Audit File for Tax"},
            {"name": "Intrastat Dispatches", "action": "eu-generate-intrastat-dispatches", "description": "Goods sent to other EU states"},
            {"name": "Intrastat Arrivals", "action": "eu-generate-intrastat-arrivals", "description": "Goods received from other EU states"},
            {"name": "EN 16931 E-Invoice", "action": "eu-generate-einvoice-en16931", "description": "European e-invoice standard"},
            {"name": "OSS Return", "action": "eu-generate-oss-return", "description": "One Stop Shop quarterly return"},
...
ACTIONS = {
    "eu-seed-eu-defaults": seed_eu_defaults,
    "eu-setup-eu-vat": setup_eu_vat,
    "eu-seed-eu-coa": seed_eu_coa,
    "eu-validate-eu-vat-number": validate_eu_vat_number,
    "eu-validate-iban": validate_iban,
    "eu-validate-eori": validate_eori,
    "eu-check-vies-format": check_vies_format,
    "eu-compute-vat": compute_vat,
    "eu-compute-reverse-charge": compute_reverse_charge,
    "eu-list-eu-vat-rates": list_eu_vat_rates,
    "eu-compute-oss-vat": compute_oss_vat,
    "eu-check-distance-selling-threshold": check_distance_selling_threshold,
    "eu-triangulation-check": triangulation_check,
    "eu-generate-vat-return": generate_vat_return,
    "eu-generate-ec-sales-list": generate_ec_sales_list,
    "eu-generate-saft-export": generate_saft_export,
    "eu-generate-intrastat-dispatches": generate_intrastat_dispatches,
    "eu-generate-intrastat-arrivals": generate_intrastat_arrivals,
    "eu-generate-einvoice-en16931": generate_einvoice_en16931,
    "eu-generate-oss-return": generate_oss_return,
    "eu-compute-withholding-tax": compute_withholding_tax,
    "eu-list-eu-countries": list_eu_countries,
    "eu-list-intrastat-codes": list_intrastat_codes,
    "eu-tax-summary": eu_tax_summary,
    "eu-available-reports": available_reports,
    "status": status,
}
