"""Shared helper functions for ERPClaw Region UK unit tests."""
import argparse
...
def load_db_query():
    ...
    spec = importlib.util.spec_from_file_location("db_query_uk", db_query_path)
...
def ns(**kwargs) -> argparse.Namespace:
    defaults = {
        "company_id": None, "amount": None, "gross_amount": None,
        "rate_type": None, "vat_number": None,
        "gross_turnover": None, "category": None, "first_year": None,
        "annual_income": None, "annual_salary": None, "plan": None,
        "region": None, "pay_period": None,
        "period": None, "month": None, "year": None, "tax_year": None,
        "employee_id": None,
        "utr": None, "nino": None, "crn": None,
...
def seed_company(conn, name="Test Co", abbr="TC", country="GB") -> str:
    cid = _uuid()
    conn.execute(
...
def seed_fiscal_year(conn, company_id: str,
                     start="2025-04-06", end="2026-04-05") -> str:
    fid = _uuid()
    conn.execute(
...
def build_env(conn) -> dict:
    cid = seed_company(conn, country="GB")
