...
def seed_uk_defaults(conn, args):
    ...
    conn.commit()
    audit(conn, "erpclaw-region-uk", "uk-seed-uk-defaults", "company", cid,
...
def setup_vat(conn, args):
    ...
    conn.commit()
    audit(conn, "erpclaw-region-uk", "uk-setup-vat", "company", cid,
...
def seed_uk_coa(conn, args):
    ...
    conn.commit()
    audit(conn, "erpclaw-region-uk", "uk-seed-uk-coa", "company", cid, new_values={"accounts": created})
    conn.commit()
    ok({
...
def seed_uk_payroll(conn, args):
    ...
    conn.commit()
    audit(conn, "erpclaw-region-uk", "uk-seed-uk-payroll", "company", cid,
...
def available_reports(conn, args):
    ...
    ok({
        "reports": [
            {"name": "VAT Return", "action": "uk-generate-vat-return", "description": "9-box MTD-compatible VAT return"},
            {"name": "MTD Payload", "action": "uk-generate-mtd-payload", "description": "HMRC MTD JSON structure"},
            {"name": "EC Sales List", "action": "uk-generate-ec-sales-list", "description": "NI Protocol sales to EU"},
            {"name": "FPS", "action": "uk-generate-fps", "description": "Full Payment Submission (RTI)"},
            {"name": "EPS", "action": "uk-generate-eps", "description": "Employer Payment Summary"},
            {"name": "P60", "action": "uk-generate-p60", "description": "End-of-year certificate"},
            {"name": "P45", "action": "uk-generate-p45", "description": "Leaver certificate"},
...
ACTIONS = {
    "uk-seed-uk-defaults": seed_uk_defaults,
    "uk-setup-vat": setup_vat,
    "uk-seed-uk-coa": seed_uk_coa,
    "uk-seed-uk-payroll": seed_uk_payroll,
    "uk-validate-vat-number": validate_vat_number,
    "uk-validate-utr": validate_utr,
    "uk-validate-nino": validate_nino,
    "uk-validate-crn": validate_crn,
    "uk-compute-vat": compute_vat,
    "uk-compute-vat-inclusive": compute_vat_inclusive,
    "uk-list-vat-rates": list_vat_rates,
    "uk-compute-flat-rate-vat": compute_flat_rate_vat,
    "uk-generate-vat-return": generate_vat_return,
    "uk-generate-mtd-payload": generate_mtd_payload,
    "uk-generate-ec-sales-list": generate_ec_sales_list,
    "uk-compute-paye": compute_paye,
    "uk-compute-ni": compute_ni,
    "uk-compute-student-loan": compute_student_loan,
    "uk-compute-pension": compute_pension,
    "uk-payroll-summary": uk_payroll_summary,
    "uk-generate-fps": generate_fps,
    "uk-generate-eps": generate_eps,
    "uk-generate-p60": generate_p60,
    "uk-generate-p45": generate_p45,
    "uk-compute-cis-deduction": compute_cis_deduction,
    "uk-tax-summary": uk_tax_summary,
    "uk-available-reports": available_reports,
    "status": status,
}
